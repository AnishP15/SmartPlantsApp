...
class SoilHumidityViewController: UIViewController {
    ...
    func setUpUI(plant: String) {
        db?.collection("plants").document(plant).getDocument(completion: { (snapshot, err) in
            let data = snapshot?.data()
            let max = data!["maxHumidity"]
            let min = data!["minHumidity"]
            
            self.maxLbl.text = "The maximum optimal value for this plant is \(max!) in Celsius"
        ...
        db?.collection(userID!).document(plant).getDocument(completion: { (snapshot, err) in
            ...
            self.tempArray = data!["HumArray"] as! Array<String>
